...
from lighting.dataloader import project_imgs, PreloadedBigTimeDataset
...
if __name__ == "__main__":
    ...
    croco_relight = CroCoRelighting(**ckpt.get('croco_kwargs', {}), pretrained_model=ckpt['model']).to(device)
    ...
    for epoch in range(300):
        for i, batch in enumerate(dataloader):
            ...
            img1_relit, img2_relit, static1, static2, img1_delit, img2_delit = croco_relight(img1, img2)
            ...
            loss = loss_relight + 0.2 * loss_static_latents #+ 0.2 * loss_delight
            loss.backward()
            croco_optim.step()
            ...
            if i == 0:
                ...
                # 0 2: img1 gt
                out_img[:res, 2 * res:, :] = img2_relit[0].permute(1, 2, 0).detach().cpu().numpy()
                ...
                # 1 2: img2 gt
                out_img[res:, 2 * res:, :] = img1_relit[0].permute(1, 2, 0).detach().cpu().numpy()
                out_img = out_img * np.array(img_std).reshape((1, 1, -1)) + np.array(img_mean).reshape((1, 1, -1))
                plt.imshow(out_img)
                plt.show()

    torch.save(croco_relight.state_dict(), "models/croco_relight_delight.pth")
