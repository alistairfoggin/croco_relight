...
def readFrames():
    ...
    subdir_name = str(videoFile).split("/")[-1].split(".")[-2]
    subdir_path = os.path.join(root, destination_dir, subdir_name)
    print(subdir_path, root, destination_dir, subdir_name)
    ...
    while(cap.isOpened()):
        ...
        if ret and read_counter % frame_step == 0:
            ...
            pil_im_resize.save(os.path.join(root, destination_dir, subdir_name, str(image_counter) + '.' + destination_format))
            image_counter += 1
        elif not ret:
            break
        read_counter += 1
...
def main(argv):
    ...
    if os.path.isdir(videoFile):
        print("Directory:", videoFile)
        filenames = os.listdir(videoFile)
        original_video_file = videoFile
        for file in filenames:
            videoFile = os.path.join(str(original_video_file), file)
            print(videoFile)
